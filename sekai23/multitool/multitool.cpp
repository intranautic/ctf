...
// routine to call for the coroutine
class Algo 
{
    public:
        Algo() {
...
// amount of times we can call an algorithm, fastalgo init with +1 call
...
// sso optimization leak potentially here?
...
auto run_algo_async(Algo* algo)
{
    ...
    {
        Algo* algo;;
        // slow algo initial ready() is false, requires two calls?
        bool await_ready() {
          dbg(__PRETTY_FUNCTION__);
          return false;
        }
...
auto do_nothing()
{
    ...
    struct awaitable
    {
        ...
        void await_resume() {dbg(__PRETTY_FUNCTION__);}
    };
    return awaitable{};
}
...
struct SavedTask {
    Task task;
    Algo* algo;
  // TODO: SSO optimization leak, we can print the name?
  // needs to be stack allocated
  // nevermind all savedtask is heap allocated
    std::string name;
